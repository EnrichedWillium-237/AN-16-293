...
void fig4() {
    ...
    for (int i = 0; i<ncbins; i++) {
        N1MCp22SUB2[i] = (TGraphErrors *) fin->Get(Form("N1MCp22SUB2/-2.4_-0.4/%d_%d/gA",cmin[i],cmax[i]));
        N1MCm22SUB2[i] = (TGraphErrors *) fin->Get(Form("N1MCm22SUB2/0.4_2.4/%d_%d/gA",cmin[i],cmax[i]));
    ...
    h1->GetYaxis()->SetRangeUser(-0.042, 0.30);
    h1->Draw();
    for (int i = 0; i<=5; i++) {
        N1MC22SUB2_syst[i]->Draw("same 2");
    }
    for (int i = 0; i<=5; i++) {
        N1MC22SUB2[i]->Draw("same p");
    }
    ...
    tx1->AddText("0.4 < |#eta| < 2.4");
    tx1->Draw();

    TLegend * leg1 = new TLegend(0.63, 0.78, 0.97, 0.90);
    SetLegend(leg1, 18);
    leg1->SetNColumns(2);
    for (int i = 0; i<=5; i++) {
    ...
    TLegend * leg2 = new TLegend(0.04, 0.78, 0.43, 0.90);
    SetLegend(leg2, 18);
    leg2->SetNColumns(2);
    for (int i = 6; i<ncbins; i++) {
